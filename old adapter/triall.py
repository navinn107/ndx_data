...
class RestAPI:
    ...
    def fetch_clients(self, client_id):
        ...
        while attempts < max_retries:
            try:
                # Ensure the PostgreSQL connection is established
                ...
                # Reconnect to PostgreSQL in case of connection issues
                self.connect_postgresql()
            
            except psycopg2.Error as e:
                # Handle all other psycopg2 errors
                print(f"PostgreSQL error occurred: {str(e)}")
                break

        # Return None if the client is not found or an error occurred
        return None
        ...
        if existing_user:
            ...
            if client_secret == existing_user[2]:
                ...
                payload = {
                    ...
                    'iss': self.iss,
                    'aud': self.aud,
                    'jti': f"{self.jti_prefix}-{str(uuid.uuid4())}",
                    'role': self.role,
                    'user': self.user,
                    'drl': self.drl
                }
                token = jwt.encode(payload, self.secretKey, algorithm=self.algo)
                return jsonify({"statusCode": 200, "detail": "JWT token generated", "token": token}), 200
            else:
                return jsonify({"statusCode": 400, "detail": "Invalid clientSecretKey."}), 400
        else:
            return jsonify({"statusCode": 400, "detail": "Invalid clientID."}), 400
    ...
    def get_data(self):
        ...
        #     return jsonify({"statusCode": 400, "detail": "Token has expired."}), 400
